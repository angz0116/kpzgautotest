...
class 获取绑定支付平台(unittest.TestCase):
	...
	def test_body(self):
		...
		print("获取绑定支付平台，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("wallet" + os.sep + "binding.json")
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否获取绑定支付平台成功"))
