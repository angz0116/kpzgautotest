...
import time
#/wallet/checkbinding
interfaceNo = "checkbinding"
name = "校验支付平台绑定 "

req = ConfigHttp()

@paramunittest.parametrized(*get_xls("interfaces.xls", interfaceNo))
class 校验支付平台绑定(unittest.TestCase):
	...
	def test_body(self):
		...
		#type支付类型：1.微信，2.支付宝，3.银联
		self.type = get_excel("type", self.No, interfaceNo)
		print("校验支付平台绑定，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("wallet" + os.sep + "checkbinding.json")
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否校验支付平台绑定成功"))
