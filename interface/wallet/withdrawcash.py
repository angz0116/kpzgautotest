...
import time
#/wallet/withdrawcash
interfaceNo = "withdrawcash"
name = "获取提现信息 "

req = ConfigHttp()

@paramunittest.parametrized(*get_xls("interfaces.xls", interfaceNo))
class 获取提现信息(unittest.TestCase):
	def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例,url, money, type, 预期结果):
		self.No = str(No)
		self.url = str(url)
	...
	def test_body(self):
		...
		# 金额
		self.money = get_excel("money", self.No, interfaceNo)
		# 类型
		self.type = get_excel("type", self.No, interfaceNo)
		print("获取提现信息，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("wallet" + os.sep + "withdrawcash.json")
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否获取提现信息成功"))
