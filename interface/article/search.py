...
class 获取热门搜索(unittest.TestCase):
	def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, search, 预期结果):
		self.No = str(No)
		self.url = str(url)
	...
	def test_body(self):
		...
		# 搜索内容
		self.search = get_excel("search", self.No, interfaceNo)
		print("获取热门搜索，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("article" + os.sep + "search.json")
		self.data["search"] = self.search
		print(self.data)
