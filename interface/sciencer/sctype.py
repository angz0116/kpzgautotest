...
class 获取科普员类型信息(unittest.TestCase):
	def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例,url, stno, 预期结果):
		self.No = str(No)
		self.stno = str(stno)
		self.url = str(url)
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否获取科普员类型信息成功"))
	...
	def wr_excel(self):
		if len(self.response["data"])>1:
			self.stno = random.choice(self.response["data"])["st_no"]
			set_excel(self.stno, "stno", self.No, interfaceNo)
			# 把科学员认证信息写入“scienceregister”
			set_excel(self.stno, "sciencetype", self.No, "scienceregister")
