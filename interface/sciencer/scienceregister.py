...
interfaceNo = "scienceregister"
name = "科普员认证"

req = ConfigHttp()

@paramunittest.parametrized(*get_xls("interfaces.xls", interfaceNo))
class 科普员认证(unittest.TestCase):
	...
	def test_body(self):
		...
		print("科普员认证接口__scienceregister，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("sciencer" + os.sep + "scienceregister.json")
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否科普员认证成功"))
