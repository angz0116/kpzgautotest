...
class 获取市区信息(unittest.TestCase):
	def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例,url,provincecode, 预期结果):
		self.No = str(No)
		self.url = str(url)
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否获取市区信息成功"))
	...
	def wr_excel(self):
		if "info" in self.response["data"]:
			dinfo = self.response["data"]["info"]
			self.logger.info(dinfo)
		else:
			self.citykeyvalue = random.choice(self.response["data"])
			# 城市编码
			self.citycode = self.citykeyvalue["code"]
			# 城市名称
			self.city = self.citykeyvalue["name"]
			set_excel(self.citycode,"citycode",self.No, "county")
			# 把code，name写入“scienceregister”中
			set_excel(self.citycode, "citycode", self.No, "scienceregister")
			set_excel(self.city, "city", self.No, "scienceregister")
