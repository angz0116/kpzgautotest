...
import time
#/category/updatechannel
interfaceNo = "updatechannel"
name = "修改设备收藏资讯频道 "

req = ConfigHttp()

@paramunittest.parametrized(*get_xls("interfaces.xls", interfaceNo))
class 修改设备收藏资讯频道(unittest.TestCase):
	def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, atid, 预期结果):
		self.No = str(No)
		self.url = str(url)
		self.atid = str(atid)
	...
	def test_body(self):
		...
		print("修改设备收藏资讯频道，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("category" + os.sep + "updatechannel.json")
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否修改设备收藏资讯频道成功"))
