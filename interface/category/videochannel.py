...
interfaceNo = "videochannel"
name = "获取视频设备收藏资讯频道"

req = ConfigHttp()

@paramunittest.parametrized(*get_xls("interfaces.xls", interfaceNo))
class 获取视频设备收藏资讯频道(unittest.TestCase):
	...
	def test_body(self):
		...
		print("获取视频设备收藏资讯频道，token==" + str(self.token))
		# 获取json字符串
		self.data = jsondata("category" + os.sep + "videochannel.json")
		print(self.data)
	...
	def check_result(self):
		try:
			self.assertEqual(self.retcode, 0, self.logger.info("检查是否获取视频设备收藏资讯频道成功"))
	...
	def wr_excel(self):
		if "data" in self.response:
			self.atid = random.choice(self.response["data"])["at_id"]
			set_excel(self.atid, "atid", self.No, "videolist")
